...
init([]) ->
    ...
    case application:get_env(luxon_reloader, interval, ?DEFAULT_INTERVAL) of
        0 ->
            % 0 means don't do it.
            {ok, #state{last = stamp()}};
        Interval ->
            {ok, TimerRef} = timer:send_interval(Interval, doit),
            {ok, #state{last = stamp(), timer_ref = TimerRef}}
    end.
...
doit(From, To) ->
    [begin
        case filelib:last_modified(FileName) of
            MTime when MTime >= From, MTime < To ->
                do_reload(Module);
            {error, enoent} ->
